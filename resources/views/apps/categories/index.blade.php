    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
        <div class="space-y-2">
            ...
            <p class="text-gray-600 font-medium">
                Kelola kategori produk dengan mudah dan efisien
            </p>
        </div>

        <div class="flex items-center gap-3">
            <!-- Search Box Modern -->
            <form method="GET" action="{{ route('categories.index') }}" class="relative hidden sm:block">
                <input type="text"
                       name="search"
        ...
        <div class="card-modern p-6 hover:scale-105 transition-transform duration-300">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $categories->sum(fn($cat) => $cat->products->count()) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="card-modern p-6 hover:scale-105 transition-transform duration-300">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $categories->where('created_at', '>=', now()->subDays(7))->count() }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card-modern overflow-hidden animate-fade-in-up" style="animation-delay: 0.2s">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100/50">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <h3 class="text-lg font-semibold text-gray-900 font-poppins">Daftar Kategori</h3>
                    @if(request('search'))
                ...
                <div class="flex items-center gap-3">
                    <!-- Mobile Search -->
                    <form method="GET" action="{{ route('categories.index') }}" class="relative sm:hidden">
                        <input type="text"
                               name="search"
                               value="{{ request('search') }}"
                               placeholder="Cari..."
            ...
            <table class="w-full table-fixed">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th class="w-80 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kategori</th>
                        <th class="w-48 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Slug</th>
                        <th class="w-24 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Produk</th>
                        <th class="w-32 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal</th>
                        <th class="w-32 px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors duration-200 group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-10 h-10 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl flex items-center justify-center flex-shrink-0">
                                    <svg class="w-5 h-5 text-gray-600" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z"/>
                                    </svg>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-gray-900 group-hover:text-primary-600 transition-colors duration-200 truncate" title="{{ $category->name }}">
                                        {{ $category->name }}
                                    </p>
                                    <p class="text-sm text-gray-500 truncate">Kategori Produk</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 truncate" title="{{ $category->slug }}">
                                {{ Str::limit($category->slug, 25) }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm whitespace-nowrap">
                                ...
                                <p class="text-gray-500">{{ $category->created_at->format('H:i') }}</p>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                @else
                                    ...
                                    <div class="text-center">
                                        <p class="text-lg font-semibold text-gray-900 mb-1">Belum ada kategori</p>
